...
class LiveReconciler:
    ...
    def _check_market_resolved(self, condition_id: str, market_id: str = "") -> Optional[dict]:
        ...
        Args:
            condition_id: Hex condition ID (for on-chain operations)
            market_id: Gamma numeric market ID (for API lookup)
        """
        resp = None
        source = "none"
        ...
        if market_id:
            try:
                ...
                log.warning(f"[RECON] Gamma /markets/{market_id} failed: {e}")
        ...
        if resp is None and market_id:
            try:
                ...
                log.warning(f"[RECON] Gamma ?id={market_id} failed: {e}")
...
def _to_bool(val) -> bool:
    ...
    Handles: True, "true", "True", "TRUE", 1, "1", "yes"
    """
    if val is None:
        return False
